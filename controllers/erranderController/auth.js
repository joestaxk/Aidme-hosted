...
const { acceptableGender,  comparePassword, filterObjectData, createKeyToken, hashPassword } = require("../../utils/helpers")
...
authController.login = async function(req, res, next) {
  ...
  authController.forgetPassword = async function(req, res, next) {
    ...
    try {
        ...
        const keyToken = await createKeyToken(Errander, config, ifExist.id)
...
authController.updatePasswordByLink = async function(req,res) {
    ...
    try {
        ...
        const findToken = await Errander.findOne({keyToken: token});
       ...
       /** if(!(await comparePassword(oldPassword.trim(), findToken.password)))  {
        ...
        const updated = await Errander.findByIdAndUpdate(findToken.id, {$set: {password: (await hashPassword(newPassword)), keyToken: "", nextChangePassword: timestamp}})
